            ...
            <div>
              ...
              <div className="space-y-0.5">
                {subtask.allowedFiles.map((file, i) => (
                  <div
                    key={i}
                    className="rounded bg-surface-raised px-2 py-0.5 font-mono text-sm text-text-secondary"
                  >
                    {file}
                  </div>
                ))}
...
function SubtaskLogViewer({ subtaskId }: { subtaskId: string }) {
  const logs = useQuery("sandbox/logs:listBySubtask" as any, { subtaskId });
