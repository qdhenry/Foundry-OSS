...
export function DiscoveryFindingsReview({
  ...
  const requirements = useQuery("requirements:listByProgram" as any, {
    programId,
  });

  const reviewFinding = useMutation("discoveryFindings:reviewFinding" as any);
  const bulkReview = useMutation("discoveryFindings:bulkReviewFindings" as any);
  const importApprovedFindings = useMutation("discoveryFindings:importApprovedFindings" as any);
  const mergeFindingIntoRequirement = useMutation(
    "discoveryFindings:mergeFindingIntoRequirement" as any,
  );
  const revertImport = useMutation("discoveryFindings:revertImport" as any);
  const acquireLock = useMutation("discoveryFindings:acquireLock" as any);
  const releaseLock = useMutation("discoveryFindings:releaseLock" as any);
      ...
      <div className="p-5">
            ...
            <button
              ...
              className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${
                resolvedActiveTab === tab
                  ? "bg-interactive-active text-accent-default"
        ...
        <div className="mb-4 flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            disabled={pendingInCurrentTab.length === 0 || busyFindingId !== null}
            className="btn-danger rounded-lg px-3 py-1.5 text-xs font-medium disabled:opacity-60"
          >
            Reject All In Tab
          </button>
