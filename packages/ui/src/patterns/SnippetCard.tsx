...
export function SnippetCard({ snippet }: SnippetCardProps) {
  const upvote = useMutation("sourceControl/patterns/snippetStorage:upvoteSnippet" as any);
  const flag = useMutation("sourceControl/patterns/snippetStorage:flagSnippet" as any);
  ...
  return (
    <div className="rounded-xl border border-border-default bg-surface-default p-4">
      ...
      <p className="mt-1 line-clamp-2 text-xs text-text-secondary">{snippet.description}</p>
      ...
      <div className="mt-3 flex items-center gap-3 border-t border-border-default pt-3">
        <button
          onClick={() => void upvote({ snippetId: snippet._id as any })}
        ...
        <button
          onClick={() => void flag({ snippetId: snippet._id as any })}
