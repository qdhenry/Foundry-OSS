...
export function ExecuteSkillModal({
  ...
  const skills = useQuery("skills:listByProgram" as any, programId ? { programId } : "skip");
  const workstreams = useQuery(
    "workstreams:listByProgram" as any,
    programId ? { programId } : "skip",
  );

  const executeSkill = useAction("ai:executeSkill" as any);
  ...
  async function handleExecute() {
    ...
    try {
      const res = await executeSkill({
        programId,
        skillId: selectedSkillId as string,
        taskPrompt,
        taskType,
        orgId,
        ...(workstreamId ? { workstreamId: workstreamId as string } : {}),
