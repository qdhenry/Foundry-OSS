...
describe("ReAnalyzeDialog", () => {
  ...
  it("renders dialog with document name in heading", () => {
    render(<ReAnalyzeDialog {...defaultProps} />);
    expect(screen.getByText("Re-analyze spec.pdf")).toBeInTheDocument();
  });
  ...
  it("fills instructions when suggestion is clicked", () => {
    render(<ReAnalyzeDialog {...defaultProps} />);
