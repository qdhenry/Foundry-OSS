...
export function TaskDecompositionPanel({ requirementId, programId }: TaskDecompositionPanelProps) {
  const data = useQuery(api.taskDecomposition.getLatestDecomposition, {
    requirementId,
  });

  const generateTasks = useMutation(api.taskDecomposition.requestDecomposition);
  const acceptDecomposition = useMutation(api.taskDecomposition.acceptDecomposition);
  const rejectDecomposition = useMutation(api.taskDecomposition.rejectDecomposition);
      ...
      <div className="rounded-xl border border-blue-200 bg-status-info-bg p-6">
        <div className="flex flex-col items-center py-6">
          ...
          <p className="mt-1 text-xs text-accent-default">
            AI is analyzing the requirement and generating tasks. This typically takes 10-30
            seconds.
          </p>
        </div>
      </div>
