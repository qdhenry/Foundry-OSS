...
interface SubtaskPanelProps {
  ...
  onExecuteAll?: () => void;
  onExecuteSelected?: (subtaskIds: Id<"subtasks">[]) => void;
  onExecuteSubtask?: (subtaskId: Id<"subtasks">, title: string, prompt: string) => void;
...
export function SubtaskPanel({
  ...
  const generateSubtasks = useMutation(apiAny.subtaskGeneration.requestSubtaskGeneration);
  const generationStatus = useQuery(apiAny.subtaskGeneration.getGenerationStatus, { taskId });
  ...
  const _startSubtaskExecution = useAction(apiAny.sandbox?.orchestrator?.startSubtaskExecution);
  const continueAfterPause = useAction(apiAny.sandbox?.orchestrator?.continueAfterPause);
  const _retrySubtask = useAction(apiAny.sandbox?.orchestrator?.retryFailedSubtask);
  ...
  async function handleRemove(subtaskId: Id<"subtasks">) {
    await removeSubtask({ subtaskId });
  }

  async function handleTitleSave(subtaskId: Id<"subtasks">, newTitle: string) {
          ...
          {hasSubtasks && !isExecuting && (
            <div className="mt-4 flex items-center gap-2 border-t border-border-default pt-3">
              ...
              {!isPaused && !isExecutionActive && selectedIds.size > 0 && onExecuteSelected && (
                <>
                  <button
                    onClick={() => onExecuteSelected(Array.from(selectedIds) as Id<"subtasks">[])}
                    className="flex items-center gap-1.5 rounded-lg bg-accent-default px-3 py-1.5 text-xs font-medium text-text-on-brand hover:bg-accent-strong"
                  >
                    <Play className="h-3.5 w-3.5" />
