...
interface EditDescriptionModalProps {
  prId: Id<"sourceControlPullRequests">;
  initialBody: string;
  onClose: () => void;
...
function RequestReviewPopover({ prId, onClose, onRequest }: RequestReviewPopoverProps) {
  ...
  const getCandidates = useAction(
    apiAny.sourceControl.tasks.prActionsInternal?.getReviewerCandidates,
  );
  ...
  async function handleRequest() {
    ...
    try {
      ...
      setError(e.message ?? "Failed to request review");
    } finally {
      setRequesting(false);
    }
  }
...
export function HeroPRCard({ taskId }: HeroPRCardProps) {
  ...
  const pr = useQuery(apiAny.sourceControl.tasks.prLifecycle.getActiveHeroPR, { taskId });

  // Mutations — may be undefined until prActions.ts is deployed
  const promoteToReady = useMutation(apiAny.sourceControl.tasks.prActions?.promoteToReady);
  const editDescription = useMutation(apiAny.sourceControl.tasks.prActions?.editDescription);
  const requestReview = useMutation(apiAny.sourceControl.tasks.prActions?.requestReview);
  const merge = useMutation(apiAny.sourceControl.tasks.prActions?.merge);
  const closePR = useMutation(apiAny.sourceControl.tasks.prActions?.close);
  const reopenPR = useMutation(apiAny.sourceControl.tasks.prActions?.reopen);
  const triggerAIReview = useAction(apiAny.sourceControl.tasks.prActionsInternal?.triggerAIReview);
  const regenerateDescription = useAction(
    apiAny.sourceControl.tasks.prActionsInternal?.regenerateDescription,
  );
  ...
  async function handlePromoteDraft() {
    ...
    try {
      ...
      setActionError(e.message ?? "Failed to promote PR");
    } finally {
  ...
  async function handleMerge(strategy: MergeStrategy) {
    ...
    try {
      ...
      setActionError(e.message ?? "Failed to merge PR");
    }
  }
  ...
  async function handleClose() {
    ...
    try {
      ...
      setActionError(e.message ?? "Failed to close PR");
    } finally {
      setClosingPR(false);
    }
  }
  ...
  async function handleReopen() {
    ...
    try {
      ...
      setActionError(e.message ?? "Failed to reopen PR");
    } finally {
  ...
  async function handleAIReview() {
    ...
    try {
      ...
      setActionError(e.message ?? "Failed to start AI review");
    } finally {
  ...
  async function handleAISummary() {
    ...
    try {
      ...
      setActionError(e.message ?? "Failed to generate AI summary");
    } finally {
