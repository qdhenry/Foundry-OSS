...
function BranchFilesSection({ taskId }: { taskId: Id<"tasks"> }) {
  ...
  const listBranchFiles = useAction(apiAny.sourceControl.tasks.prActionsInternal?.listBranchFiles);
  ...
  const handleToggle = useCallback(async () => {
    ...
    try {
      ...
      setError(resolveErrorMessage(e, "Failed to load files"));
    } finally {
      setLoading(false);
    }
...
export function TaskImplementationPanel({ taskId }: TaskImplementationPanelProps) {
  ...
  const heroPR = useQuery(apiAny.sourceControl.tasks.prLifecycle.getActiveHeroPR, { taskId });

  // Branch info — tells us if a branch exists before any PR is created
  const branchInfo = useQuery(apiAny.sandbox.sessions.getBranchInfoForTask, { taskId });

  // refreshFromGitHub action
  const refreshFromGitHub = useAction(
    apiAny.sourceControl.tasks.prActionsInternal?.refreshFromGitHub,
  );
  const [refreshing, setRefreshing] = useState(false);
  const [refreshError, setRefreshError] = useState<string | null>(null);

  // syncBranchActivity action
  const syncBranch = useAction(apiAny.sourceControl.tasks.prActionsInternal?.syncBranchActivity);
  ...
  async function handleRefresh() {
    ...
    try {
      ...
      setRefreshError(resolveErrorMessage(e, "Failed to refresh"));
    } finally {
      setRefreshing(false);
    }
  }
  ...
  async function handleSyncBranch() {
    ...
    try {
      ...
      if (result?.status === "no_branch" || result?.status === "branch_not_found") {
        setSyncResult(result.message);
      } else if (result?.synced > 0 || result?.prScheduled) {
        if (result?.prScheduled) {
          setPrPending(true);
          setSyncResult(`Synced ${result.synced} commits`);
        } else {
          setSyncResult(`Synced ${result.synced} commits`);
        }
      } else {
      ...
      setSyncResult(resolveErrorMessage(e, "Sync failed"));
    } finally {
      setSyncing(false);
    }
  }
  ...
  const commits: any[] = heroPR.commits ?? [];
  ...
  return (
    <div className="space-y-3">
      ...
      {heroPR.filesChanged > 0 && (
        <ChangedFilesSection
          prId={heroPR._id}
          filesChanged={heroPR.filesChanged}
          additions={heroPR.additions}
          deletions={heroPR.deletions}
        />
      )}
