...
export function DependencySuggestions({ programId }: DependencySuggestionsProps) {
  const suggestions = useQuery(api.dependencyDetection.getPendingSuggestions, {
    programId,
  });
  const approveDep = useMutation(api.dependencyDetection.approveDependency);
  const dismissDep = useMutation(api.dependencyDetection.dismissDependency);

  const [loadingIds, setLoadingIds] = useState<Set<string>>(new Set());

  const handleApprove = async (depId: Id<"workstreamDependencies">) => {
  ...
  const handleDismiss = async (depId: Id<"workstreamDependencies">) => {
    setLoadingIds((prev) => new Set(prev).add(depId));
    try {
  ...
  return (
    <section>
      ...
      <div className="space-y-3">
        {suggestions.map((suggestion: DependencySuggestion) => {
