...
app.post("/", async (c) => {
  const { prompt, changes, existingRisks, context } = await c.req.json();
...
Respond with valid JSON matching the required schema.`;
  ...
  const result = await runAgentQuery(
    ...
    { prompt, systemPrompt, maxThinkingTokens: 6000 },
    c.env.ANTHROPIC_API_KEY,
  );

  return c.json({ risk_assessment: result.data, metadata: result.metadata });
});

export { app as evaluateRisksRoute };
