...
router.post("/", async (req, res, next) => {
  try {
    ...
    if (!taskTitle) {
      res.status(400).json({
        error: { code: "MISSING_TASK", message: "taskTitle is required" },
      });
      return;
    }
...
Analyze and produce proposals for each subtask as JSON.`;
    ...
    const result = await runAgentQuery(TaskAnalysisResultSchema, {
      prompt,
      systemPrompt,
      model,
...
export { router as analyzeTaskSubtasksRouter };
