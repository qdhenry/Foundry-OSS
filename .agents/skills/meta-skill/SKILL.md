...
description: Creates new Agent Skills for Codex following best practices and documentation. Use when the user wants to create a new skill, extend Codex's capabilities, or package domain expertise into a reusable skill.
---
...
This skill helps you create new Agent Skills for Codex. Before starting, read the comprehensive documentation files in the [docs/](docs/) directory for complete context.
...
- Instructions that Codex loads on-demand when relevant
...
2. When should Codex use this skill? (triggers)
...
Create skills in the project's `.Codex/skills/` directory for team sharing:
...
**Note:** Project skills (`.Codex/skills/`) are automatically shared with your team via git. For personal skills only you use, create in `~/.Codex/skills/` instead.
...
## Instructions
[Clear, step-by-step guidance for Codex]
...
**Optional Frontmatter (Codex only):**
- `allowed-tools`: Restrict which tools Codex can use (e.g., `Read, Grep, Glob`)
...
- Return clear output for Codex to parse
...
- Make it clear when Codex should read vs execute files
...
3. Create the skill directory:
   ```bash
   mkdir -p .Codex/skills/python-code-review
   ```
...
3. Create structure:
   ```bash
   mkdir -p .Codex/skills/csv-analysis/scripts
   mkdir -p .Codex/skills/csv-analysis/templates
   ```
...
3. Create comprehensive structure:
   ```bash
   mkdir -p .Codex/skills/tech-docs/{templates,examples,guidelines}
   ```
4. Organize content:
...
1. Read current skill:
   ```bash
   cat .Codex/skills/transcribe/SKILL.md
   ```
...
3. Create supporting file:
   ```bash
   touch .Codex/skills/transcribe/spell_corrections.md
   ```
...
4. **Test thoroughly** - Verify Codex discovers and uses the skill correctly
5. **Iterate with feedback** - Refine based on actual usage

Skills transform general-purpose Codex into a specialist for your domain. Start small, test early, and expand as needed.
