...
Enables AI agents (Codex, Cursor) to interact with and debug the Foundry Tauri desktop app through screenshots, DOM access, input simulation, and more via the Model Context Protocol.

## When to Use

- Setting up the MCP plugin in the Tauri app
- Configuring Codex or Cursor to connect to the running desktop app
...
Add to `.Codex/settings.json` or project MCP config:
...
Add the MCP server config to your Codex settings (`.Codex/settings.json`) or Cursor settings.
